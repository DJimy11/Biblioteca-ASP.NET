    ...
    public class PrestamoLogica
    {
        ...
        public bool Registrar(Prestamo objeto)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("sp_RegistrarEsmprestimo", oConexion);
        ...
        public bool Existe(Prestamo objeto)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("sp_existeEmprestimo", oConexion);
                    cmd.Parameters.AddWithValue("IdPersona", objeto.oPersona.IdPersona);
                    cmd.Parameters.AddWithValue("IdLibro", objeto.oLibro.IdLibro);
                    cmd.Parameters.Add("Resultado", SqlDbType.Bit).Direction = ParameterDirection.Output;
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public List<EstadoPrestamo> ListarEstados()
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("select IdEstadoPrestamo,Descripcion from ESTADO_EMPRESTIMO", oConexion);
        ...
        public List<Prestamo> Listar(int idestadoprestamo, int idpersona)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    ...
                    query.AppendLine("select p.IdPrestamo,ep.IdEstadoPrestamo,ep.Descripcion,pe.Codigo,pe.Nombre,pe.Apellido,li.Titulo,convert(char(10),p.FechaDevolucion,103)[FechaDevolucion],convert(char(10),p.FechaConfirmacionDevolucion,103)[FechaConfirmacionDevolucion],p.EstadoEntregado,p.EstadoRecibido from EMPRESTIMO p");
                    query.AppendLine("inner join estado_emprestimo ep on ep.IdEstadoPrestamo = p.IdEstadoPrestamo");
                    query.AppendLine("inner join PESSOA pe on pe.IdPersona = p.IdPersona");
                    query.AppendLine("inner join LIVRO li on li.IdLibro = p.IdLibro");
        ...
        public bool Devolver(string estadorecibido, int idprestamo)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                ...
                {
                    StringBuilder query = new StringBuilder();
                    query.AppendLine("update EMPRESTIMO set IdEstadoPrestamo = 2 ,FechaConfirmacionDevolucion = GETDATE(),EstadoRecibido =@estadorecibido");
