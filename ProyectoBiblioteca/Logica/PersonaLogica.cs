    ...
    public class PersonaLogica
    {
        ...
        public bool Registrar(Persona objeto)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("sp_RegistrarPessoa", oConexion);
        ...
        public bool Modificar(Persona objeto)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("sp_ModificarPessoa", oConexion);
        ...
        public List<Persona> Listar()
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    ...
                    sb.AppendLine("select p.IdPersona,p.Nombre,p.Apellido,p.Correo,p.Clave,p.Codigo,tp.IdTipoPersona,tp.Descripcion, p.Estado from pessoa p");
                    sb.AppendLine("inner join TIPO_PESSOA tp on tp.IdTipoPersona = p.IdTipoPersona");
        ...
        public bool Eliminar(int id)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("delete from pessoa where IdPersona = @id", oConexion);
